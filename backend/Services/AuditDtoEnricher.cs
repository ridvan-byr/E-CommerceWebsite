...
public static class AuditDtoEnricher
{
    public static async Task EnrichAsync<T>(
        ...
        CancellationToken cancellationToken = default) where T : IAuditable
    {
        if (items.Count == 0)
            return;
    ...
    public static Task EnrichProductsAsync(
        IReadOnlyList<ProductResponseDto> items,
        IUserRepository users,
        CancellationToken cancellationToken = default)
    {
        return EnrichAsync(items, users, cancellationToken);
    }

    public static Task EnrichCategoriesAsync(
